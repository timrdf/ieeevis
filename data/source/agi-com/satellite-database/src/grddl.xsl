...
<!-- 
   Thanks to http://p2p.wrox.com/xslt/85114-how-parse-html-contents-inside-cdata-elements.html 
   for pointing out David Carlisle's http://web-xslt.googlecode.com/svn/trunk/htmlparse/htmlparse.xsl
   (also mentioned http://www.saxonica.com/documentation/extensions/functions/parse-html.xml, but less direct) -->

<xsl:output method="text"/>
<!--xsl:output method="xml" indent="yes" omit-xml-declaration="yes"/ for XDL checks -->

<!-- https://github.com/timrdf/csv2rdf4lod-automation/wiki/SDV-organization -->
<xsl:param name="cr-base-uri"   select="'http://ieeevis.tw.rpi.edu'"/>
<xsl:param name="cr-source-id"  select="'agi-com'"/>
<xsl:param name="cr-dataset-id" select="'satellite-database'"/>
<xsl:param name="cr-version-id" select="'latest'"/>

<xsl:variable name="base" select="'http://adn.agi.com/SatelliteDatabase/KmlNetworkLink.aspx'"/>

<xsl:template match="/">
   <xsl:value-of select="concat('AGI KML Identifier,International Designator,Satellite Number,SSC,',
                                'Name,',
                                'Launch Site,Launch Date,',
                                'Owner,Folder,',
                                'Period (minutes),Perigee (km),Apogee (km),Inclination (deg),Satellite Type/Orbit Type,',
                                'Position X,Position Y,Altitude,Altitude Mode,',
                                'Active,',
                                'Celstrak Page',
                                $NL)"/>

   <!--html:html for XDL checks><html:body-->
...
<xsl:template match="kml:Placemark">
   ...
   <!-- As of Nov 2013, the HTML descriptions look like:
         <b>Satellite Number</b> 15644<br/>
         <b>International Designator</b> 1985-028D<br/>
         <b>Owner</b> US<br/>
         <b>Active</b> False<br/>
         <b>Launch Date</b> 4/12/1985 1:59:00 PM<br/>
         <b>Launch Site</b> AFE<br/>
         <b>Period</b> 539.6 minutes<br/>
         <b>Perigee</b> 322 km<br/>
         <b>Apogee</b> 30834 km<br/>
         <b>Inclination</b> 22.8999999947713 deg<br/>
         <b>
            <a href="..\KmlTrajectoryRequest.aspx?SSC=15644&amp;Period=32376"
               type="application/vnd.google-earth.kmz">Display Trajectory in Fixed Frame</a>
         </b>
         <br/>
         <br/>
         <a href="http://www.agi.com/">
            <img border="0" src="logo_small.gif"/>
         </a> -->
   <!--html:div for XDL checks>
   <xsl:copy-of select="d:htmlparse(kml:description)"/>
   </html:div-->
   <xsl:variable name="descriptions" select="d:htmlparse(kml:description)"/>

   <xsl:variable name="id"                       select="@id"/>
   <xsl:variable name="international-designator" select="normalize-space($descriptions/html:b[.='International Designator']/following-sibling::text()[1])"/>
   <xsl:variable name="satellite-number"         select="normalize-space($descriptions/html:b[.='Satellite Number']/following-sibling::text()[1])"/>
   <xsl:variable name="ssc"                      select="normalize-space(replace(replace($descriptions/html:b/html:a[contains(@href,'SSC=')]/@href,'^.*\?SSC=',''),'&amp;.*$',''))"/>
   <xsl:variable name="celestrak-page"           select="normalize-space($descriptions/html:b/html:a[contains(@href,'celestrak.com')]/@href)"/>
   <xsl:variable name="name"                     select="kml:name"/>
   <xsl:variable name="launch-site"              select="normalize-space($descriptions/html:b[.='Launch Site']/following-sibling::text()[1])"/>
   <xsl:variable name="launch-date"              select="normalize-space($descriptions/html:b[.='Launch Date']/following-sibling::text()[1])"/>
   <xsl:variable name="owner"                    select="normalize-space($descriptions/html:b[.='Owner']/following-sibling::text()[1])"/>
   <xsl:variable name="folder"                   select="../kml:name"/>
   <xsl:variable name="period"                   select="normalize-space($descriptions/html:b[.='Period']/following-sibling::text()[1])"/>
   <xsl:variable name="perigee"                  select="normalize-space($descriptions/html:b[.='Perigee']/following-sibling::text()[1])"/>
   <xsl:variable name="apogee"                   select="normalize-space($descriptions/html:b[.='Apogee']/following-sibling::text()[1])"/>
   <xsl:variable name="inclination"              select="normalize-space($descriptions/html:b[.='Inclination']/following-sibling::text()[1])"/>
   <xsl:variable name="altitude-mode"            select="kml:Point/kml:altitudeMode"/>
   <xsl:variable name="x"                        select="tokenize(kml:Point/kml:coordinates,',')[1]"/>
   <xsl:variable name="y"                        select="tokenize(kml:Point/kml:coordinates,',')[2]"/>
   <xsl:variable name="a"                        select="tokenize(kml:Point/kml:coordinates,',')[3]"/>
   <xsl:variable name="active"                   select="normalize-space($descriptions/html:b[.='Active']/following-sibling::text()[1])"/>
   <xsl:variable name="orbit-active"             select="replace(kml:styleUrl,'^#','')"/>

   <xsl:value-of select="concat($DQ,string-join((
                                                 $id,$international-designator,$satellite-number,$ssc,
                                                 $name,
                                                 $launch-site,$launch-date,
                                                 $owner,$folder,
                                                 $period,$perigee,$apogee,$inclination,$orbit-active,
                                                 $x,$y,$a,$altitude-mode,
                                                 $active,
                                                 $celestrak-page
                                                ),
                                                concat($DQ,',',$DQ)),$DQ,$NL)"/>
   <!-- replace($description,$DQ,concat('\\',DQ)) 
